    ...
    for(int i = 0; i < CATEGORY; i++)
    {
        ...
        for(int j = 0; j < DAYS; j++)
        {
            ...
            catch(int exception)
            {
                ...
                pounds[i][j] = exception; //replaces the negative variable with a positive variable once a positive number is successfully entered.
                sum += pounds[i][j]; //Determines the sum of all the food eaten for each individual category.
                command::space();
                o << days[j] << pounds[i][j]; information += o.str();  //Collects the information which will be later transferred into the document[] array.
                o.str(""); //Clears the ostringstream operator.
                o.clear(); //Clears any error flags that may have generated.
            }
        }
