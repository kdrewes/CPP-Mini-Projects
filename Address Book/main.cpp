...
void Find(vector<std::string>&AddressBookString, vector<AddressBook>&AddressBookVector, string firstName, string lastName, bool&valid)
{
    ...
    {
        if((AddressBookString[i].find(firstName) != string::npos) && (AddressBookString[i].find(lastName) != string::npos))
        {
    ...
    if(!valid){cout << firstName << ' ' << lastName << " could not be found.\n";}
...
void Input()
{
   ...
   case 1:
        ...
        do
    {
        ...
        cout << "\n----------- Address Book: -----------\n";
               writeFile.open("AddressBook.txt", ios::in | ios::binary);
               readFile.open("AddressBook.txt", ios::in | ios::binary);
               if(!readFile)
               {
           ...
           case 2:
           {
               ...
               if(valid)
               {
               writeFile.open("AddressBook.txt", ios::out | ios::binary | ios::trunc);
